...
#' @return \code{pmvinvbeta} gives the cumulative probability.
...
#'
#' @export
pmvinvbeta <- function(q, parm1 = 1, parm2 = rep(1, k), algorithm = "Numerical", nsim = 1e7) {
...
#' @return \code{qmvinvbeta} gives the equicoordinate quantile. The result is seed dependent if Monte Carlo algorithm is chosen  (\code{algorithm = "MC"}).
#' @examples
#' \donttest{
...
#' @export
qmvinvbeta <- function(p, parm1 = 1, parm2 = rep(1, k), interval = c(1e-8, 1e8), algorithm = "Numerical", nsim = 1e6) {
  ...
  if (algorithm == "Numerical") {
    rootfun <- function(x) {
      equi.x <- rep(x, k)
      val <- pmvinvbeta(equi.x, parm1 = parm1, parm2 = parm2, algorithm = "Numerical", nsim = nsim) - p
    }
  }
  if (algorithm == "MC") {
    rootfun <- function(x) {
    equi.x <- rep(x, k)
    val <- pmvinvbeta(equi.x, parm1 = parm1, parm2 = parm2, algorithm = "MC", nsim = nsim) - p
    }
  } 
...
#' @rdname MvtInvBeta
#' @return \code{smvinvbeta} gives the value of survival function.
#' @examples
#' smvinvbeta(q = c(2, 4), parm1 = 7, parm2 = c(1, 3)) # Survival function
#'
#' @export
smvinvbeta <- function(q, parm1 = 1, parm2 = rep(1, k), algorithm = "Numerical", nsim = 1e7) {
  if (any(q < 0) ||  !is.vector(q, mode = "numeric")) {
    stop(sQuote("q"), " must be a vector of non-negative quantiles")
  }
  k <- length(q)
  ...
  for (i in 1:k) {
    ...
    for (j in 1:card) {
      ...
      sub.cdf[j] <- pmvinvbeta(sub.q, parm1 = parm1, parm2 = sub.parm2, algorithm = algorithm, nsim = nsim)
    }
