...
#' @name MvtF
#' @description Calculation of density function, cumulative distribution function, equicoordinate quantile function and survival function, and random numbers generation for multivariate F distribution with degrees of freedom \code{df}.
#'
...
#' @return \code{pmvglomax} gives the cumulative probability.
...
#'
#' @export
pmvf <- function(q, df = rep(1, k + 1), algorithm = "Numerical", nsim = 1e7) {
  ...
  if (algorithm == "MC") {
    ...
    for (i in 1:k) {
      trueTable <- trueTable[trueTable[,i] == TRUE, ]
    }
    if (k == 1) {
      return(sum(trueTable == TRUE) / nsim)
    }
    else {
      return(nrow(trueTable) / nsim)
    }
  }
  else {
	stop(sQuote("algorithm"), " must be either Numerical or MC")
  }
}
...
#' @return \code{qmvf} gives the equicoordinate quantile. The result is seed dependent if Monte Carlo algorithm is chosen  (\code{algorithm = "MC"}).
#' @examples
#' \donttest{
...
#' @export
qmvf <- function(p, df = rep(1, k + 1), interval = c(1e-8, 1e8), algorithm = "Numerical", nsim = 1e6) {
  ...
  if (algorithm == "Numerical") {
    rootfun <- function(x) {
      equi.x <- rep(x, k)
      val <- pmvf(equi.x, df = df, algorithm = "Numerical") - p
    }
  }
  if (algorithm == "MC") {
    rootfun <- function(x) {
      equi.x <- rep(x, k)
      val <- pmvf(equi.x, df = df, algorithm = "MC", nsim = nsim) - p
    }
  } 
  if (! algorithm %in% c("Numerical", "MC")){
    stop(sQuote("algorithm"), " must be either Numerical or MC")
  }
...
#' @rdname MvtF
#' @return \code{smvf} gives the value of survival function.
#' @examples
#' smvf(q = c(1, 2), df = c(2, 4, 6)) # Survival function
#'
#' @export
smvf <- function(q, df = rep(1, k + 1), algorithm = "Numerical", nsim = 1e7) {
  if (any(q < 0) ||  !is.vector(q, mode = "numeric")) {
    stop(sQuote("q"), " must be a vector of non-negative quantiles")
  }
  k <- length(q)
  ...
  for (i in 1:k) {
    ...
    for (j in 1:card) {
      ...
      sub.vn <- vn[sub.index]
      sub.cdf[j] <- pmvf(sub.q, df = c(v0, sub.vn), algorithm = algorithm, nsim = nsim)
    }
